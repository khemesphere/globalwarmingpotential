...
```{r fig3, fig.height= 4, fig.width=6, dpi = 600}
corn_savings <- emission_savings(time = 100, baseline_co2 = 826.2, baseline_co2sd = 163, co2 = -424, co2sd = 78.7, ch4 = 48, ch4sd = 6.9, type = "fe", runs = 1000)
pasture_savings <- emission_savings(time = 100, baseline_co2 = 305.7, baseline_co2sd = 78.7, co2 = -424, co2sd = 78.7, ch4 = 48, ch4sd = 6.9, type = "fe", runs = 1000)

ggplot() +
  geom_hline(yintercept = 0) +
  geom_ribbon(data = corn_savings, aes(x = time, ymin = (net_mean - net_sd)/1000, ymax = (net_mean + net_sd)/1000), fill = "light grey" , alpha = 0.5) +
  geom_line(data = corn_savings, aes(x = time, y = net_mean/1000)) +
  geom_ribbon(data = pasture_savings, aes(x = time, ymin = (co2base_cum - co2base_sd)/1000, ymax = (co2base_cum + co2base_sd)/1000), fill = "orange" , alpha = 0.5) +
  geom_line(data = pasture_savings, aes(x = time, y = co2base_cum/1000), color = "orchid") + 
  geom_ribbon(data = corn_savings, aes(x = time, ymin = (co2base_cum - co2base_sd)/1000, ymax = (co2base_cum + co2base_sd)/1000), fill = "light green" , alpha = 0.5) +
  geom_line(data = corn_savings, aes(x = time, y = co2base_cum/1000), color = "green") +  
  ylab("GWP* CO2eq. emissions (kg m-2)") + xlab("Time since restoration (years)") +
  annotate("text", x = 75, y = 200, label = "Corn (100 yr): 87 kg CO2 m-2") +
  annotate("text", x = 75, y = 185, label = "Pasture (100 yr): 18 kg CO2 m-2") +
  theme_bw()
